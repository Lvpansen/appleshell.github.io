...
目前 JS 中最大的数是`Math.pow(2,53) - 1`，也可以通过`Number.MAX_SAFE_INTEGER`获得，顾名思义，这个数是 js 中可以安全递增的最大整数，超出这个数字之外的计算可能存在精度丢失的问题。
...
ES2020 不要求统一属性遍历顺序，而是对遍历过程中的一些特殊 Case 明确定义了一些规则：
