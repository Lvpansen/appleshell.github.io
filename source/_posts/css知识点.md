...
通俗理解就是一个页面的所有样式来源中，哪个个才是最终起作用的。因为样式来源有多种：浏览器默认样式（也叫用户代理样式）、页面作者（程序员）写的样式、浏览器用户自定义样式（这个不常见）。
...
参看[这篇文章](https://www.cnblogs.com/chen-cong/p/7862832.html)，只看定义不太好理解。
