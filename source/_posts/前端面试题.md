...
obj.method(fn, 1)
```
上段代码输出啥？如果把let换成var呢？

这道题涉及的到点：this的指向问题，let和var对全局变量的影响，window.length属性的指向，arguments变量。
...
[Javascript引擎如何工作](https://segmentfault.com/a/1190000019205065)，很好的一篇讲解Javascript原理的文章，涉及全局内存、调用栈、执行环境上下文、回调队列和事件循环等。

6. ajax 原理，手写实现。
  
7. 闭包

    理解闭包，需要先理解**执行上下文**和**词法作用域**

    《你不知道的JS》中对闭包有详细的介绍。[这篇文章](https://medium.com/dailyjs/i-never-understood-javascript-closures-9663703368e8)对闭包的解释也比较详细。
...
18. 数据类型转换
    ...
    [参考文章](https://javascript.ruanyifeng.com/grammar/conversion.html)

19. 柯里化

    [参考文章](https://blog.bitsrc.io/understanding-currying-in-javascript-ceb2188c339)
...
11. 深入JSX
    ...
    JSX仅仅是`React.createElement(component, props, ...children)`函数的语法糖。在React项目之所以要每个组件引入React，就是因为JSX会被编译成React.createElement的形式。

    props：

      - 如果每个prop赋值，它的默认值就是`true`。下面两个JSX等价：
        ```jsx
        <MyInput disabled />

        <MyInput disabled={true} />
        ```
      - 可以使用`...`展开运算符在JSX中传递整个props对象，下面两个组件等价：
        ```jsx
        function App() {
          return <Hello firstName="Jack" lastName="Ma" />
        }

        function App() {
          const nameInfo = {firstName: 'Jack', lastName: 'Ma'}
          return <Hello {...nameInfo} />
        }
        ```
        当然，也可以在传递其他属性的同时，使用展开运算符传递其他属性。
        ```jsx
        function App {
          const nameInfo = {firstName: 'Jack', lastName: 'Ma'}
          return <Hello className={'hello'} {...nameInfo}>
        }
        ```
        实际上，之所以能这样传，是因为在被babel编译为React.createElement时，props参数是由`{className: 'hello'}`和`nameInfo`对象通过`Object.assgin`方法合并得到（不支持ES6语法的情况下，通过循环合并）

    组件的子元素是字符串字面量时，JSX会把行首尾的空格、空行移除。两行文本之间的空行会被压缩成一个空格

    false、null、undefined作为子元素时不会被渲染，有一些转为boolea为false的值，例如0，则仍会被渲染。

12. React性能优化

    - 打包构建层面的优化，每种打包工具都有对应的优化插件（例如压缩）。生产环境使用生产版本的React。
    - 长列表的渲染通过使用虚拟滚动的技术进行优化，有限的时间渲染有限的内容。
    - shouldComponentUpdate或React.PureComponent的使用，优化渲染流程。

13. 协调

    React中，render()函数执行时创建了一棵元素组成的树，在state或者porps更新后，render()返回一棵不同的树。React需要计算如何高效率的更新UI来匹配最新的树。

    React的这个计算基于两个假设：

      1. 两个类型不同的元素会生成两棵不同的树；
      2. 卡法这通过key属性能够暗示哪些子元素可以在不同的渲染过程中保持稳定。

    diff算法

    计算两棵树的不同时，React首先比较两个根元素。基于根元素类型的异同，变现是不同的。

    1. 元素类型不同

    React会拆除旧的树，从头构建新的树。

    拆除旧树时，旧的DOM节点也被销毁。组件实例执行`componentWillUnmount()`生命周期函数。创建新树时，新的DOM节点被插入到DOM中。组件实例依次执行`componentWillMount()`和`componentDidMount()`生命周期函数。任何和旧树相关联的状态被丢弃。

    2. 元素类型相同

        - 相同类型的DOM元素

          比较两个相同类型的DOM元素时，React会保持底层DOM节点不变，关注的是两者的属性，只更新变化的属性。

          处理完DOM节点，React开始对子元素进行递归。

        - 相同类型的组件元素

          当组件更新时，组件实例保持不变，这是为了在渲染过程中维持状态（state is maintained）。React更新底层实例的props来匹配新的元素，在底层实例中调用`componentWillReceiveProps()`和`componentWillUpdate()`生命周期函数。

          然后，render()函数被调用，diff算法在旧的和新的结果上执行递归，计算差异。

    3. 子元素的递归

        默认情况下，对一个DOM节点子元素进行递归，React只是同时遍历两个子元素列表，产生差异时，生成一个mutation。

        下例中，React生成一个mutation
        ```html
        <ul>
          <li>first</li>
          <li>second</li>
        ...
        ```
        下例中，React生成三个mutation。React并不知道first和second是保持不变的。开销会比较大。
        ```html
        <ul>
          <li>Duke</li>
          <li>Villanova</li>
    ...
    4. keys
        ...
        给子元素设置key属性可以解决这个问题。React使用key来匹配新旧两棵树中的子元素。上例中，如果每个li元素有一个key属性，React就知道first和second连个子元素是不变的，只是发生了移动。

        在子元素列表中，key必须是唯一的。

        也可以使用子元素的index作为key，但前提是子元素不会被重新排序，即每个元素的index不能变化。

        因为重新排序会对组件的状态（state）产生影响。组件实例通过key来进行更新和复用。如果组件的key是index，当移动一个子元素导致key发生变化，组件的状态，例如非受控组件，会发生混乱，导致不可预期的更新。

14. Fiber

    [参考文章](https://indepth.dev/inside-fiber-in-depth-overview-of-the-new-reconciliation-algorithm-in-react/)。另外这个网站其他文章也可以看看
